...
- The raw configuration fil produced in the previous step does not run out of the box due to some issue with the `offline Tau reco treee` sequence. This part need to be manually commented out for smooth running. Add this line to the end of `egcheck.py` 
```diff
...
```
Use `egcheck.py` for submitting **crab** jobs to make the Ntuples.
