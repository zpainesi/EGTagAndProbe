...
void produceResolution(string infile,string ofileName, TString treeName,string prefix="",bool doEmulation=false, bool doAllRuns=false ,std::vector<Int_t> RunNumbers = std::vector<Int_t>(),Long64_t maxEvents=100000, Int_t reportEvery=100)
{
	...
	Double_t PtEdges[PtBINS + 1] = {5., 10., 14., 18., 20., 23., 26., 30., 40., 50., 60., 70., 80., 90., 100., 140.0,300.0};
    ...
    for( auto idx : RunNumbers) {
        ...
        resolutionMap[idx]["GenEtVsPFEtoveGenEt"  ] = new resolutionMeasurement("GenEtVsPFEtoveGenEt",PtBINS,PtEdges,dPtBINS,dPtEdges);
        resolutionMap[idx]["GenEtVsSCEtoveGenEt"  ] = new resolutionMeasurement("GenEtVsSCEtoveGenEt",PtBINS,PtEdges,dPtBINS,dPtEdges);
    ...
    t1->SetBranchAddress("isBremSafe",&isBremSafe);
    t1->SetBranchAddress("isDaughterOfZ",&isDaughterOfZ);
    t1->SetBranchAddress("genClosesetZDauEt",&genClosesetZDauEt);
  ...
  //     std::cout<<" isBremSafe : "<<isBremSafe<<"\n";
  //     if(isBremSafe < 0.5) continue ;
  //     if(isDaughterOfZ < 0.5) continue ;
  //     if(Nvtx >52) continue;   
       ...
       if( doAllRuns)
       {
        ...
        //if (eleProbePt < 12.0) 
        //    std::cout<<eleProbeSclEt<<" , "<<eleProbeSclEt<<"\n";
        resolutionMap[RunNumber]["GenEtVsPFEtoveGenEt"  ]->Fill(  eleProbeEta,genPt,eleProbePt/genPt  );
        resolutionMap[RunNumber]["GenEtVsSCEtoveGenEt"  ]->Fill(  eleProbeEta,genPt,eleProbeSclEt/genPt  );
...
//infile=/grid_mnt/t3storage3/athachay/l1egamma/data/run3MC/TagAndProbe_ReEmul_122Xsample_CaloParams_2022_v0_2_withPFA1p_recaliberatedZDauGenEt_v0_p1.root
