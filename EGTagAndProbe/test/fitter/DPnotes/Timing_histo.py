...
if __name__ == "__main__" :
    ...
    inFile = ROOT.TFile('/eos/user/z/zpainesi/forL1DPS2023July/Timing_EG_2023.root')
    ...
    spike_TH1 = inFile.Get('eg_BX')
    label = '$p^{L1}_{T}>15$ GeV & $p^{L1}_{T}<26$ GeV\n$p^{off}_{T}>12$ GeV & $p^{off}_{T}<23$ GeV\nTotal pre-firing fraction 0.202%\nTotal post-firing fraction 0.180%'
    ...
    #plt.errorbar(x, y, xerr=x_err, yerr=y_err, ls='None', label=label, marker='s', lw=2, color=cmap(imap), zorder=zorder)
    ...
    for xtick in ax.xaxis.get_major_ticks():
        xtick.set_pad(10)
    #leg = plt.legend(loc = 'upper right', fontsize=18, title=r'$|\eta^{e/\gamma}|<2.5$')
    ...
    plt.yscale('log')
    plt.grid()
    plot_name = 'various/Timing_EG_2023_' 
